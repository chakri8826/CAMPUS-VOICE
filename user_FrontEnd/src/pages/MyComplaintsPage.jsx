    ...
    <div className="min-h-screen bg-[#10231c] dark group/design-root overflow-x-hidden" style={{ fontFamily: 'Plus Jakarta Sans, Noto Sans, sans-serif' }}>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)}>
        <form onSubmit={handleSubmit} className="bg-[#18382c] rounded-lg shadow-lg p-8 w-full max-w-lg space-y-6 z-10 border border-[#214a3c]">
          ...
          <button type="submit" disabled={submitting} className="w-full py-2 bg-[#019863] text-white font-semibold rounded hover:bg-[#017a4f] transition">
      ...
      <div className="py-8 px-2">
        <h2 className="text-2xl font-bold text-[#8ecdb7] mb-6 text-center">My Complaints</h2>
      <div className="max-w-3xl mx-auto grid gap-6">
        ...
        {complaints.map(c => (
          <div 
            key={c._id} 
            className="bg-[#18382c] rounded-lg shadow p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border border-[#214a3c] cursor-pointer hover:bg-[#214a3c] transition-colors"
            onClick={() => navigate(`/complaints/${c._id}`)}
          >
            <div className="flex-1">
              <div className="text-lg font-semibold mb-1 text-white">{c.title}</div>
              <div className="text-sm text-[#8ecdb7] mb-2">Submitted: {new Date(c.createdAt).toLocaleDateString()}</div>
              {c.adminNotes && (
                <div className="text-sm text-green-400 bg-green-900/30 rounded px-2 py-1 inline-block mt-1">Admin: {c.adminNotes}</div>
              )}
                  ...
                  {c.attachments.map((file, idx) => {
                    ...
                    return isImage ? (
                      ...
                      >
                        ...
                        />
                        <div className="text-xs text-[#8ecdb7] text-center px-2 py-1 truncate bg-[#18382c]">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-block px-3 py-1 bg-[#019863] text-white rounded hover:bg-[#017a4f] text-xs font-medium transition"
                        onClick={(e) => e.stopPropagation()}
                      >
                        {file.originalName || file.filename}
                      </a>
            ...
            <div className={`px-3 py-1 rounded-full text-sm font-bold self-start md:self-center ${statusColors[c.status]}`}>{c.status.replace('_', ' ').toUpperCase()}</div>
          </div>
        ))}
      </div>
      </div>
